...
export const CaringTaskPage = (props: Props) => {
  ...
  const fertilizer_columns = [
    {
      ...
      render: (value: any) => (
        <TextField
          value={
            fertilizers?.find((x) => x.id === value)?.name || "Không xác định"
          }
        />
      ),
    },
  ...
  const handleFertilizerChange = (value: any) => {
    const existingFertilizer = chosenFertilizers?.find(
      (x) => x.fertilizer_id === value
    );
